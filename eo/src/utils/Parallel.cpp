...
namespace eo
{
	...
	if ( doMeasure() )
		...
		eoLogger log;
		log << file("measure_" + prefix()) << _t_end - _t_start << std::endl;
	    }
#endif // !_OPENMP
    }
    ...
    {
	parallel._createParameters( parser );

#ifdef _OPENMP
	if ( parallel.isEnabled() )
	    {
		if ( parallel.nthreads() > 0 )
		    {
			omp_set_num_threads( parallel.nthreads() );
		    }
	    }

	if ( parallel.doMeasure() )
	    {
		parallel._t_start = omp_get_wtime();
	    }
#endif // !_OPENMP
    }
